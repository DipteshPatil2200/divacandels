...
adminRouter.get("/dashboard", asyncHandler(async (_req, res) => {
  const [totalProducts, activeProducts, categories, lowStock, outOfStock, bulkProducts, newInquiries, contactInquiries, productInquiries, bulkInquiries, corporateInquiries, amazonClicks, whatsappClicks, recentInquiries, recentProducts, recentBulkRequests] = await db.$transaction([
    db.product.count({ where: { deletedAt: null } }), db.product.count({ where: { isActive: true, deletedAt: null } }), db.category.count({ where: { isActive: true } }), db.product.count({ where: { stockQuantity: { gt: 0, lte: 5 }, deletedAt: null } }), db.product.count({ where: { stockQuantity: 0, deletedAt: null } }), db.product.count({ where: { isBulkAvailable: true, deletedAt: null } }), db.inquiry.count({ where: { status: "NEW" } }), db.inquiry.count({ where: { source: "WEBSITE_CONTACT", status: "NEW" } }), db.inquiry.count({ where: { inquiryType: "PRODUCT", status: "NEW" } }), db.inquiry.count({ where: { inquiryType: "BULK_ORDER", status: "NEW" } }), db.inquiry.count({ where: { inquiryType: "CORPORATE_GIFTING", status: "NEW" } }), db.clickEvent.count({ where: { eventType: "AMAZON" } }), db.clickEvent.count({ where: { eventType: "WHATSAPP" } }), db.inquiry.findMany({ take: 6, orderBy: { createdAt: "desc" } }), db.product.findMany({ where: { deletedAt: null }, take: 6, include: { images: { orderBy: { displayOrder: "asc" } } }, orderBy: { createdAt: "desc" } }), db.inquiry.findMany({ where: { inquiryType: "BULK_ORDER" }, take: 6, orderBy: { createdAt: "desc" } })
  ]);
...
adminRouter.post("/database/cleanup", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => {
  ...
  await audit(req, "LOG_CLEANUP", "ApplicationLog", "application_logs", undefined, { deletedLogs: result.deletedLogs, retentionDays: result.after.cleanup.retentionPolicy.applicationLogDays });
  res.json({ success: true, data: result });
}));

adminRouter.post("/logs/cleanup", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => {
  const result = await runSafeDatabaseCleanup();
  await audit(req, "LOG_CLEANUP", "ApplicationLog", "application_logs", undefined, { deletedLogs: result.deletedLogs, retentionDays: result.after.cleanup.retentionPolicy.applicationLogDays });
  res.json({ success: true, data: result });
}));

adminRouter.get("/categories", asyncHandler(async (_req, res) => res.json({ success: true, data: await db.category.findMany({ orderBy: { displayOrder: "asc" } }) })));
adminRouter.post("/categories", asyncHandler(async (req, res) => { const input = categorySchema.parse(req.body); const data = await db.category.create({ data: input }); await audit(req, "CREATE", "Category", String(data.id), undefined, data); res.status(201).json({ success: true, data }); }));
adminRouter.patch("/categories/:id", asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.category.findUniqueOrThrow({ where: { id } }); const data = await db.category.update({ where: { id }, data: categorySchema.partial().parse(req.body) }); await audit(req, "UPDATE", "Category", String(id), previous, data); res.json({ success: true, data }); }));
adminRouter.delete("/categories/:id", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => { const id = Number(req.params.id); if (await db.product.count({ where: { categoryId: id, deletedAt: null } })) throw new AppError(409, "Move or delete products in this collection first", "CATEGORY_IN_USE"); const previous = await db.category.delete({ where: { id } }); await audit(req, "DELETE", "Category", String(id), previous); res.json({ success: true }); }));

adminRouter.get("/team", asyncHandler(async (_req, res) => res.json({ success: true, data: await db.teamMember.findMany({ orderBy: [{ displayOrder: "asc" }, { id: "asc" }] }) })));
adminRouter.post("/team", asyncHandler(async (req, res) => { const input = teamSchema.parse(req.body); const data = await db.teamMember.create({ data: input }); await audit(req, "CREATE", "TeamMember", String(data.id), undefined, data); res.status(201).json({ success: true, data }); }));
adminRouter.patch("/team/:id", asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.teamMember.findUniqueOrThrow({ where: { id } }); const input = teamSchema.partial().parse(req.body); const data = await db.teamMember.update({ where: { id }, data: input }); await audit(req, "UPDATE", "TeamMember", String(id), previous, data); res.json({ success: true, data }); }));
adminRouter.delete("/team/:id", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.teamMember.findUniqueOrThrow({ where: { id } }); if (previous.profileImageUrl) await deleteStoredImage({ imageUrl: previous.profileImageUrl, cloudinaryPublicId: previous.cloudinaryPublicId }); await db.teamMember.delete({ where: { id } }); await audit(req, "DELETE", "TeamMember", String(id), previous); res.json({ success: true }); }));
adminRouter.post("/team/:id/photo", imageUpload.single("image"), asyncHandler(async (req, res) => {
  const id = Number(req.params.id); const member = await db.teamMember.findUnique({ where: { id } }); const file = req.file;
  ...
  try { const data = await db.teamMember.update({ where: { id }, data: { profileImageUrl: stored.imageUrl, cloudinaryPublicId: stored.cloudinaryPublicId } }); if (member.profileImageUrl) await deleteStoredImage({ imageUrl: member.profileImageUrl, cloudinaryPublicId: member.cloudinaryPublicId }); await audit(req, "UPLOAD_PHOTO", "TeamMember", String(id), member, data); res.json({ success: true, data }); }
  catch (error) { await deleteStoredImage(stored); throw error; }
}));
adminRouter.delete("/team/:id/photo", asyncHandler(async (req, res) => { const id = Number(req.params.id); const member = await db.teamMember.findUniqueOrThrow({ where: { id } }); if (member.profileImageUrl) await deleteStoredImage({ imageUrl: member.profileImageUrl, cloudinaryPublicId: member.cloudinaryPublicId }); const data = await db.teamMember.update({ where: { id }, data: { profileImageUrl: null, cloudinaryPublicId: null } }); await audit(req, "DELETE_PHOTO", "TeamMember", String(id), member, data); res.json({ success: true, data }); }));

adminRouter.get("/bulk-products", asyncHandler(async (_req, res) => res.json({ success: true, data: await db.product.findMany({ where: { deletedAt: null }, include: { category: true, images: { orderBy: { displayOrder: "asc" } }, bulkPricingTiers: { orderBy: { minimumQuantity: "asc" } } }, orderBy: { updatedAt: "desc" } }) })));
adminRouter.patch("/bulk-products/:id", asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.product.findUniqueOrThrow({ where: { id } }); const input = bulkSchema.partial().parse(req.body); const data = await db.product.update({ where: { id }, data: input }); await audit(req, "UPDATE_BULK", "Product", String(id), previous, data); res.json({ success: true, data }); }));
adminRouter.post("/bulk-products/:id/pricing", asyncHandler(async (req, res) => { const productId = Number(req.params.id); const input = pricingTierSchema.parse(req.body); const data = await db.bulkPricingTier.create({ data: { ...input, productId } }); await audit(req, "CREATE", "BulkPricingTier", String(data.id), undefined, data); res.status(201).json({ success: true, data }); }));
adminRouter.patch("/bulk-pricing/:id", asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.bulkPricingTier.findUniqueOrThrow({ where: { id } }); const input = pricingTierSchema.partial().parse(req.body); const data = await db.bulkPricingTier.update({ where: { id }, data: input }); await audit(req, "UPDATE", "BulkPricingTier", String(id), previous, data); res.json({ success: true, data }); }));
adminRouter.delete("/bulk-pricing/:id", asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.bulkPricingTier.delete({ where: { id } }); await audit(req, "DELETE", "BulkPricingTier", String(id), previous); res.json({ success: true }); }));

adminRouter.get("/products", asyncHandler(async (req, res) => { const deleted = req.query.deleted === "true"; const data = await db.product.findMany({ where: { deletedAt: deleted ? { not: null } : null }, include: { category: true, images: true }, orderBy: { updatedAt: "desc" } }); res.json({ success: true, data }); }));
adminRouter.get("/products/:id", asyncHandler(async (req, res) => { const data = await db.product.findUnique({ where: { id: Number(req.params.id) }, include: { category: true, images: true } }); if (!data) throw new AppError(404, "Product not found"); res.json({ success: true, data }); }));
adminRouter.post("/products", asyncHandler(async (req, res) => { const input = productSchema.parse(req.body); const data = await db.product.create({ data: productData(input) }); await audit(req, "CREATE", "Product", String(data.id), undefined, data); res.status(201).json({ success: true, data }); }));
adminRouter.patch("/products/:id", asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.product.findUniqueOrThrow({ where: { id } }); const compareAtPrice = req.body.compareAtPrice === undefined ? previous.compareAtPrice === null ? null : Number(previous.compareAtPrice) : req.body.compareAtPrice === null ? null : Number(req.body.compareAtPrice); const bulkStartingPrice = req.body.bulkStartingPrice === undefined ? previous.bulkStartingPrice === null ? null : Number(previous.bulkStartingPrice) : req.body.bulkStartingPrice === null ? null : Number(req.body.bulkStartingPrice); const merged = productSchema.parse({ ...previous, ...req.body, price: Number(req.body.price ?? previous.price), compareAtPrice, bulkStartingPrice, stockQuantity: Number(req.body.stockQuantity ?? previous.stockQuantity) }); const data = await db.product.update({ where: { id }, data: productData(merged) }); await audit(req, "UPDATE", "Product", String(id), previous, data); res.json({ success: true, data }); }));
adminRouter.delete("/products/:id", asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.product.update({ where: { id }, data: { deletedAt: new Date(), isActive: false } }); await audit(req, "SOFT_DELETE", "Product", String(id), previous); res.json({ success: true }); }));
adminRouter.post("/products/:id/duplicate", asyncHandler(async (req, res) => { const source = await db.product.findUniqueOrThrow({ where: { id: Number(req.params.id) } }); const stamp = Date.now().toString().slice(-8); const copy: Record<string, unknown> = { ...source }; delete copy.id; delete copy.createdAt; delete copy.updatedAt; const data = await db.product.create({ data: { ...copy, sku: `${source.sku.slice(0, 68)}-COPY-${stamp}`, slug: `${source.slug.slice(0, 200)}-copy-${stamp}`, name: `${source.name} Copy`, isActive: false, deletedAt: null } as any }); await audit(req, "DUPLICATE", "Product", String(data.id), source, data); res.status(201).json({ success: true, data }); }));
adminRouter.post("/products/:id/restore", asyncHandler(async (req, res) => { const id = Number(req.params.id); const data = await db.product.update({ where: { id }, data: { deletedAt: null } }); await audit(req, "RESTORE", "Product", String(id), undefined, data); res.json({ success: true, data }); }));
...
adminRouter.post("/products/:id/images", imageUpload.array("images", 8), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const shouldReplacePrimary = req.body.replacePrimary === "true" || !product.images.some((image: any) => image.isPrimary);
    const firstOrder = product.images.reduce((max: number, image: any) => Math.max(max, image.displayOrder), -1) + 1;
  ...
  const data = await db.productImage.findMany({ where: { productId }, orderBy: { displayOrder: "asc" } });
...
adminRouter.delete("/products/:id/images/:imageId", asyncHandler(async (req, res) => {
  ...
  if (image.isPrimary) {
    const next = await db.productImage.findFirst({ where: { productId }, orderBy: { displayOrder: "asc" } });
...
adminRouter.patch("/products/:id/images/reorder", asyncHandler(async (req, res) => {
  ...
  await db.$transaction(input.imageIds.map((id, displayOrder) => db.productImage.update({ where: { id }, data: { displayOrder, isPrimary: input.primaryImageId ? id === input.primaryImageId : undefined } })));
  const data = await db.productImage.findMany({ where: { productId }, orderBy: { displayOrder: "asc" } });
...
adminRouter.get("/inquiries", asyncHandler(async (req, res) => { const status = typeof req.query.status === "string" ? req.query.status : undefined; const data = await db.inquiry.findMany({ where: status ? { status } : {}, include: { product: { select: { name: true, sku: true } } }, orderBy: { createdAt: "desc" } }); res.json({ success: true, data }); }));
adminRouter.get("/inquiries/:id", asyncHandler(async (req, res) => { const data = await db.inquiry.findUnique({ where: { id: Number(req.params.id) }, include: { product: { select: { name: true, sku: true } } } }); if (!data) throw new AppError(404, "Inquiry not found", "NOT_FOUND"); res.json({ success: true, data }); }));
adminRouter.patch("/inquiries/:id", asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.inquiry.findUniqueOrThrow({ where: { id } }); const input = inquiryAdminSchema.parse(req.body); const data = await db.inquiry.update({ where: { id }, data: input }); await audit(req, "UPDATE", "Inquiry", String(id), previous, data); res.json({ success: true, data }); }));
adminRouter.delete("/inquiries/:id", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.inquiry.delete({ where: { id } }); await audit(req, "DELETE", "Inquiry", String(id), previous); res.json({ success: true }); }));
adminRouter.post("/inquiries/:id/retry-notification", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => { const id = Number(req.params.id); const inquiry = await db.inquiry.findUniqueOrThrow({ where: { id } }); await sendInquiryEmails(inquiry); const data = await db.inquiry.update({ where: { id }, data: { emailNotificationSent: true } }); await audit(req, "RETRY_NOTIFICATION", "Inquiry", String(id), inquiry, data); res.json({ success: true, data }); }));
...
adminRouter.get("/faqs", asyncHandler(async (_req, res) => res.json({ success: true, data: await db.fAQ.findMany({ orderBy: [{ displayOrder: "asc" }, { id: "asc" }] }) })));
adminRouter.post("/faqs", asyncHandler(async (req, res) => { const data = await db.fAQ.create({ data: faqSchema.parse(req.body) }); await audit(req, "CREATE", "FAQ", String(data.id), undefined, data); res.status(201).json({ success: true, data }); }));
adminRouter.patch("/faqs/:id", asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.fAQ.findUniqueOrThrow({ where: { id } }); const data = await db.fAQ.update({ where: { id }, data: faqSchema.partial().parse(req.body) }); await audit(req, "UPDATE", "FAQ", String(id), previous, data); res.json({ success: true, data }); }));
adminRouter.delete("/faqs/:id", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.fAQ.delete({ where: { id } }); await audit(req, "DELETE", "FAQ", String(id), previous); res.json({ success: true }); }));
...
adminRouter.get("/content", asyncHandler(async (_req, res) => res.json({ success: true, data: await db.pageContent.findMany({ orderBy: { pageKey: "asc" } }) })));
adminRouter.put("/content/:pageKey", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => { const pageKey = z.enum(["TERMS", "PRIVACY_POLICY", "SHIPPING_POLICY", "RETURN_REFUND_POLICY"]).parse(String(req.params.pageKey)); const input = pageContentSchema.omit({ pageKey: true }).parse(req.body); const previous = await db.pageContent.findUnique({ where: { pageKey } }); const data = await db.pageContent.upsert({ where: { pageKey }, create: { pageKey, ...input, updatedById: req.admin!.id }, update: { ...input, updatedById: req.admin!.id } }); await audit(req, "UPSERT", "PageContent", pageKey, previous, data); res.json({ success: true, data }); }));
...
adminRouter.get("/expenses", asyncHandler(async (req, res) => {
  ...
  const [data, aggregate, grouped] = await db.$transaction([
    db.expense.findMany({ where, include: { createdBy: { select: { name: true } } }, orderBy: [{ expenseDate: "desc" }, { id: "desc" }], skip, take: query.pageSize }),
    db.expense.aggregate({ where, _sum: { amount: true }, _count: true }),
    db.expense.groupBy({ by: ["category"], where, _sum: { amount: true }, _count: true, orderBy: { _sum: { amount: "desc" } } })
  ]);
  const totalPages = Math.max(1, Math.ceil(aggregate._count / query.pageSize));
  res.json({ success: true, data: { expenses: data, total: Number(aggregate._sum.amount ?? 0), count: aggregate._count, categories: grouped.map((item: any) => ({ category: item.category, total: Number(item._sum?.amount ?? 0), count: item._count })), pagination: { page: query.page, pageSize: query.pageSize, totalPages, hasPrevious: query.page > 1, hasNext: query.page < totalPages } } });
}));
adminRouter.get("/expenses/dashboard", asyncHandler(async (req, res) => {
  ...
  res.json({ success: true, data: { year, total: Number(summary?.total ?? 0), paid: Number(summary?.paid ?? 0), unpaid: Number(summary?.unpaid ?? 0), partial: Number(summary?.partial ?? 0), totalGst: Number(summary?.totalGst ?? 0), count: Number(summary?.count ?? 0), average: Number(summary?.average ?? 0), highest: Number(summary?.highest ?? 0), monthly, categories: categoryRows.map((row: any) => ({ category: row.category, total: Number(row.total) })) } });
}));
adminRouter.post("/expenses", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => { const input = expenseInputSchema.parse(req.body); const data = await db.expense.create({ data: { ...input, expenseNumber: `DIVA-EXP-${new Date().getFullYear()}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`, createdByAdminId: req.admin!.id } }); await audit(req, "CREATE", "Expense", String(data.id), undefined, data); res.status(201).json({ success: true, data }); }));
adminRouter.patch("/expenses/:id", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.expense.findUniqueOrThrow({ where: { id } }); const input = expenseInputSchema.partial().parse(req.body); const data = await db.expense.update({ where: { id }, data: input }); await audit(req, "UPDATE", "Expense", String(id), previous, data); res.json({ success: true, data }); }));
adminRouter.delete("/expenses/:id", requireRole("SUPER_ADMIN", "ADMIN"), asyncHandler(async (req, res) => { const id = Number(req.params.id); const previous = await db.expense.delete({ where: { id } }); await audit(req, "DELETE", "Expense", String(id), previous); res.json({ success: true }); }));
adminRouter.get("/expenses/report.pdf", asyncHandler(async (req, res) => {
  ...
  const grouped = await db.$queryRaw<ReportRow[]>`SELECT [category], DATEPART(MONTH, [expenseDate]) AS [month], SUM([amount]) AS [amount], COUNT_BIG(*) AS [recordCount] FROM [Expense] WHERE [expenseDate] >= ${from} AND [expenseDate] < ${to} GROUP BY [category], DATEPART(MONTH, [expenseDate]) ORDER BY [category], DATEPART(MONTH, [expenseDate])`;
