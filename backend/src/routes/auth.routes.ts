...
authRouter.post("/login", asyncHandler(async (req, res) => {
  ...
  if (!(await bcrypt.compare(input.password, user.passwordHash))) {
    ...
    await prisma.adminUser.update({ where: { id: user.id }, data: { failedLoginAttempts: attempts, lockedUntil: attempts >= 5 ? new Date(Date.now() + 15 * 60 * 1000) : null } });
    throw new AppError(401, "Invalid email or password", "INVALID_CREDENTIALS");
  }
  const refresh = setSession(res, user);
  await prisma.$transaction([
    prisma.refreshToken.create({ data: { tokenHash: hashToken(refresh), expiresAt: new Date(Date.now() + 7 * 86400000), adminId: user.id } }),
    prisma.adminUser.update({ where: { id: user.id }, data: { failedLoginAttempts: 0, lockedUntil: null, lastLoginAt: new Date() } })
  ]);
...
authRouter.post("/refresh", asyncHandler(async (req, res) => {
  ...
  const stored = await prisma.refreshToken.findUnique({ where: { tokenHash: hashToken(token) }, include: { admin: true } });
  ...
  await prisma.$transaction([
    ...
    prisma.refreshToken.create({ data: { tokenHash: hashToken(nextRefresh), expiresAt: new Date(Date.now() + 7 * 86400000), adminId: stored.adminId } })
  ]);
...
authRouter.get("/me", requireAdmin, asyncHandler(async (req, res) => {
  const user = await prisma.adminUser.findUnique({ where: { id: req.admin!.id }, select: { id: true, name: true, email: true, role: true, mustChangePassword: true } });
...
authRouter.post("/change-password", requireAdmin, asyncHandler(async (req, res) => {
  ...
  await prisma.adminUser.update({ where: { id: user.id }, data: { passwordHash: await bcrypt.hash(input.newPassword, 12), mustChangePassword: false } });
  await prisma.refreshToken.updateMany({ where: { adminId: user.id, revokedAt: null }, data: { revokedAt: new Date() } });
  res.json({ success: true });
}));
