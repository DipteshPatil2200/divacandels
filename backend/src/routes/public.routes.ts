...
publicRouter.get("/team", asyncHandler(async (_req, res) => {
  const data = await db.teamMember.findMany({ where: { isActive: true }, orderBy: [{ isFounder: "desc" }, { displayOrder: "asc" }, { id: "asc" }] });
  res.json({ success: true, data });
}));

publicRouter.get("/faqs", asyncHandler(async (_req, res) => {
  const data = await db.fAQ.findMany({ where: { isActive: true }, orderBy: [{ displayOrder: "asc" }, { id: "asc" }] });
  res.json({ success: true, data });
}));
...
publicRouter.get("/content/:pageKey", asyncHandler(async (req, res) => {
  ...
  const data = await db.pageContent.findFirst({ where: { pageKey, isPublished: true }, select: { pageKey: true, title: true, content: true, updatedAt: true } });
...
publicRouter.get("/categories", asyncHandler(async (_req, res) => {
  const data = await db.category.findMany({ where: { isActive: true }, orderBy: [{ displayOrder: "asc" }, { name: "asc" }] });
  res.json({ success: true, data });
}));

publicRouter.get("/categories/:slug", asyncHandler(async (req, res) => {
  const data = await db.category.findFirst({ where: { slug: String(req.params.slug), isActive: true }, include: { products: { where: { isActive: true, deletedAt: null }, include: { images: true } } } });
...
publicRouter.get("/products", asyncHandler(async (req, res) => {
  ...
  const [data, total] = await db.$transaction([db.product.findMany({ where, include: productInclude, orderBy, skip: (query.page - 1) * query.limit, take: query.limit }), db.product.count({ where })]);
...
for (const [path, flag] of [["featured", "isFeatured"], ["best-sellers", "isBestSeller"], ["new-arrivals", "isNewArrival"]] as const) {
  publicRouter.get(`/products/${path}`, asyncHandler(async (_req, res) => {
    const data = await db.product.findMany({ where: { isActive: true, deletedAt: null, [flag]: true }, include: productInclude, take: 8, orderBy: { updatedAt: "desc" } });
    res.json({ success: true, data });
  }));
}

publicRouter.get("/products/:slug", asyncHandler(async (req, res) => {
  const data = await db.product.findFirst({ where: { slug: String(req.params.slug), isActive: true, deletedAt: null }, include: productInclude });
  if (!data) throw new AppError(404, "Product not found", "NOT_FOUND");
  res.json({ success: true, data });
}));

publicRouter.get("/bulk-products", asyncHandler(async (_req, res) => {
  const data = await db.product.findMany({ where: { isActive: true, isBulkAvailable: true, deletedAt: null }, include: bulkProductInclude, orderBy: [{ isFeatured: "desc" }, { updatedAt: "desc" }] });
  res.json({ success: true, data });
}));
...
publicRouter.get("/banners", asyncHandler(async (_req, res) => {
  const now = new Date();
  const data = await db.banner.findMany({ where: { isActive: true, AND: [{ OR: [{ startDate: null }, { startDate: { lte: now } }] }, { OR: [{ endDate: null }, { endDate: { gte: now } }] }] }, orderBy: { displayOrder: "asc" } });
  res.json({ success: true, data });
}));

publicRouter.get("/testimonials", asyncHandler(async (_req, res) => {
  res.json({ success: true, data: await db.testimonial.findMany({ where: { isApproved: true }, orderBy: { displayOrder: "asc" } }) });
}));
...
publicRouter.post("/bulk-inquiries", asyncHandler(async (req, res) => {
  ...
  const product = await db.product.findFirst({ where: { id: input.productId, isActive: true, isBulkAvailable: true, deletedAt: null } });
...
publicRouter.post("/inquiries", asyncHandler(async (req, res) => {
  ...
  const inquiry = await db.inquiry.create({ data: { ...inquiryData, email: input.email || null, inquiryNumber } });
  let emailSent = false;
  try { await sendInquiryEmails(inquiry); emailSent = true; } catch (error) { console.error("Inquiry email failed", error); }
  if (emailSent) await db.inquiry.update({ where: { id: inquiry.id }, data: { emailNotificationSent: true } });
...
publicRouter.post("/contact-inquiries", asyncHandler(async (req, res) => {
  ...
  const draft = await db.inquiry.create({ data: { inquiryNumber: `TMP-${crypto.randomUUID()}`, inquiryType: "GENERAL", customerName: contact.customerName, phone: contact.phone, email: contact.email || null, message: contact.message, status: "NEW", source: "WEBSITE_CONTACT" } });
  const inquiryNumber = `DIVA-CONTACT-${draft.createdAt.getFullYear()}-${String(draft.id).padStart(5, "0")}`;
  const inquiry = await db.inquiry.update({ where: { id: draft.id }, data: { inquiryNumber } });
  let emailNotificationSent = false;
  try { await sendInquiryEmails(inquiry); emailNotificationSent = true; await db.inquiry.update({ where: { id: inquiry.id }, data: { emailNotificationSent: true } }); }
...
publicRouter.post("/newsletter/subscribe", asyncHandler(async (req, res) => {
  ...
  await db.newsletterSubscriber.upsert({ where: { email: email.toLowerCase() }, create: { email: email.toLowerCase() }, update: { isActive: true, unsubscribedAt: null } });
...
publicRouter.post("/analytics/click", asyncHandler(async (req, res) => {
  ...
  await db.clickEvent.create({ data: input });
  res.status(201).json({ success: true });
}));
