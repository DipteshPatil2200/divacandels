...
beforeAll(async () => { await db.inquiry.deleteMany({ where: { email: "contact-test@example.com" } }); });

afterAll(async () => {
  if (createdIds.length) await db.inquiry.deleteMany({ where: { id: { in: createdIds } } });
  await db.$disconnect();
});
...
describe("contact, FAQ and policy flows", () => {
  it("normalizes and saves a contact inquiry before returning a WhatsApp continuation", async () => {
    ...
    const saved = await db.inquiry.findUnique({ where: { inquiryNumber: response.body.data.inquiryNumber } }); expect(saved).toMatchObject({ inquiryType: "GENERAL", source: "WEBSITE_CONTACT", status: "NEW", phone: "+918421869308" }); createdIds.push(saved!.id);
    const admin = await db.adminUser.findFirstOrThrow({ where: { isActive: true } }); const authorization = `Bearer ${signToken(admin.id, admin.email, admin.role, "access")}`;
  ...
  it("allows an administrator to manage FAQs and policy content", async () => {
    ...
    const current = await db.pageContent.findUniqueOrThrow({ where: { pageKey: "TERMS" } }); const saved = await request(app).put("/api/v1/admin/content/TERMS").set("Authorization", authorization).send({ title: current.title, content: current.content, isPublished: current.isPublished }); expect(saved.status).toBe(200);
  });
});
